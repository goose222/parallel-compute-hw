...
{
    float Pvalue[4] = {0.0, 0.0, 0.0, 0.0};
    //计算Pd和Md中元素的行索引
    ...
    for(int i=0; i<y/TILE_WIDTH;i++){
        ...
        a_share[threadIdx.x][threadIdx.y+TILE_WIDTH/4]=Md[Row*y+(i*TILE_WIDTH+threadIdx.y+TILE_WIDTH/4)];
        b_share[threadIdx.y+TILE_WIDTH/4][threadIdx.x]=Nd[(Col+TILE_WIDTH/4)*y+(i*TILE_WIDTH+threadIdx.x)];

        a_share[threadIdx.x][threadIdx.y+TILE_WIDTH/4*2]=Md[Row*y+(i*TILE_WIDTH+threadIdx.y+TILE_WIDTH/4*2)];
        b_share[threadIdx.y+TILE_WIDTH/4*2][threadIdx.x]=Nd[(Col+TILE_WIDTH/4*2)*y+(i*TILE_WIDTH+threadIdx.x)];

        a_share[threadIdx.x][threadIdx.y+TILE_WIDTH/4*3]=Md[Row*y+(i*TILE_WIDTH+threadIdx.y+TILE_WIDTH/4*3)];
        b_share[threadIdx.y+TILE_WIDTH/4*3][threadIdx.x]=Nd[(Col+TILE_WIDTH/4*3)*y+(i*TILE_WIDTH+threadIdx.x)];

        __syncthreads();

        for(int n=0;n<4;n++){
            for(int k=0; k<TILE_WIDTH; k++){
                float tmp = (a_share[threadIdx.x][k]-b_share[threadIdx.y+TILE_WIDTH/4*n][k]);
                Pvalue[n] += tmp*tmp;
            }
        }
        __syncthreads();
    ...
    for(int i=0;i<4;i++){
        Pvalue[i] = exp(-0.5*Pvalue[i]/(sigma*sigma));
        Pd[Row*n_b+Col+i*TILE_WIDTH/4] = Pvalue[i];
    }
}
